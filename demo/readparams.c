...
//Corey Edit: open file for data output
  FILE *f = fopen("Motion_Data.csv", "w");
  ...
  if(!outfilter || outcnp<=0){ // no output filtering
    ...
    for(i=0; i<ncams*cnp; i+=cnp){
      ...
      for(j=0; j<outcnp; ++j){
        ...
        //Corey's Code to print SBA motion output
        fprintf(f, "%.10e", filtered[j]);
        fputc(',',f);
      }
      fputc('\n', fp);
      fputc('\n',f);
...
register int i;
  ...
  //Corey Edit: open file for data output
  FILE *f = fopen("Structure_Data.csv", "w");
